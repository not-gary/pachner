...
-- type-level finite sets
-- basics on simplicial complexes
-- basics on simplicial complexes
...
-/
@[simp]
def IsSimplicialMap (X : SimplicialComplex α) (Y : SimplicialComplex β) (f : α → β) :=
  ∀ s, s ∈ X.simplices → Finset.image f s ∈ Y.simplices

structure SimplicialMap (X : SimplicialComplex α) (Y : SimplicialComplex β) where mk ::
  map : α → β
  is_simplicial : IsSimplicialMap X Y map

theorem simplicialMap_restrict_is_simplicial (X Y : SimplicialComplex α) (Z : SimplicialComplex β)
    (f : SimplicialMap X Z) : Y ⊆ X → IsSimplicialMap Y Z f.map :=
  by
...
def SimplicialMap.restrict {X Y : SimplicialComplex α} {Z : SimplicialComplex β}
    (f : SimplicialMap X Z) (Y_sub_X : Y ⊆ X) : SimplicialMap Y Z :=
  SimplicialMap.mk f.map (simplicialMap_restrict_is_simplicial X Y Z f Y_sub_X)

def simplicialMapLift {X : SimplicialComplex α} {Y : SimplicialComplex β} (f : SimplicialMap X Y) :
    Finset α → Finset β := fun s : Finset α => Finset.image f.map s
...
theorem simplicialImage_congr {X : SimplicialComplex α} (f g : α → β) :
    Set.EqOn f g (vertices X) → (simplicialImage X f).simplices = (simplicialImage X g).simplices :=
  by
  ...
  · intro s_in_fX
    ...
    apply Set.EqOn.symm
    apply @Set.EqOn.mono _ _ _ (vertices X)
...
theorem map_is_simplicial_onto_image (X : SimplicialComplex α) (f : α → β) :
...
theorem simplicialImage_vertices (X : SimplicialComplex α) (f : α → β) :
    vertices (simplicialImage X f) = f '' vertices X :=
  by
...
notation "⟨" f ", " X "⟩" =>
  @SimplicialMap.mk _ _ _ X (simplicialImage X f) f (map_is_simplicial_onto_image X f)

theorem simplicialImage_is_lift_image (X : SimplicialComplex α) (f : α → β) :
    (simplicialImage X f).simplices = simplicialMapLift ⟨f, X⟩ '' X.simplices :=
  by
...
theorem simplicialMap_on_vertices (X : SimplicialComplex α) (Y : SimplicialComplex β)
    (f : SimplicialMap X Y) (x : α) (x_in_X : x ∈ vertices X) : f.map x ∈ vertices Y :=
  by
  simp only [vertices, Set.mem_iUnion]
...
theorem simplicial_lift_bij_implies_vertices_bij (X : SimplicialComplex α) (Y : SimplicialComplex β)
    (f : SimplicialMap X Y) :
    Set.BijOn (simplicialMapLift f) X.simplices Y.simplices →
      Set.BijOn f.map (vertices X) (vertices Y) :=
  by
...
theorem vertices_bij_implies_simplicial_lift_mapsTo (X : SimplicialComplex α)
    (Y : SimplicialComplex β) (f : SimplicialMap X Y) :
    Set.BijOn f.map (vertices X) (vertices Y) →
      Set.MapsTo (simplicialMapLift f) X.simplices Y.simplices :=
  by
  simp only [Set.BijOn, Set.MapsTo, Set.InjOn, Set.SurjOn, simplicialMapLift, vertices_setOf]
  intro img_bij
  ...
  apply simplicialComplex_empty_simplex
  simp only [Finset.eq_empty_iff_forall_not_mem, Classical.not_forall, Classical.not_not] at s_ne
...
theorem vertices_bij_implies_simplicial_lift_inj (X : SimplicialComplex α) (Y : SimplicialComplex β)
    (f : SimplicialMap X Y) :
    Set.BijOn f.map (vertices X) (vertices Y) → Set.InjOn (simplicialMapLift f) X.simplices :=
  by
  simp only [Set.BijOn, Set.MapsTo, Set.InjOn, Set.SurjOn, simplicialMapLift, vertices_setOf]
  intro img_bij
  choose img_range img_inj img_surj using img_bij
  ...
  rw [← Finset.mem_coe, Finset.coe_image, @Set.InjOn.mem_image_iff _ _ (vertices X),
...
theorem id_isSimplicialMap [DecidableEq α] (X : SimplicialComplex α) : IsSimplicialMap X X id := by
  simp

def idSimplicialMap [DecidableEq α] (X : SimplicialComplex α) : SimplicialMap X X :=
...
def SimplicialMap.comp [DecidableEq β] [DecidableEq γ] {X : SimplicialComplex α}
    ...
    (g : SimplicialMap Y Z) : SimplicialMap X Z :=
...
@[simp]
def IsSimpliciallyIso (X : SimplicialComplex α) (Y : SimplicialComplex β) : Prop :=
  ∃ f : SimplicialMap X Y, IsSimplicialIso f

infixr:50 " ≅ " => IsSimpliciallyIso

theorem simplicial_iso_implies_lift_bij (X : SimplicialComplex α) (Y : SimplicialComplex β)
    (f : SimplicialMap X Y) :
    IsSimplicialIso f → Y.simplices = simplicialMapLift f '' X.simplices :=
  by
  intro f_iso
  unfold IsSimplicialIso at f_iso
  choose g g_inv_f using f_iso
  ...
  rw [← Set.image_comp]
  conv_rhs => rw [← Set.image_id ↑t]
  apply Set.EqOn.image_eq
  simp only [Function.funext_iff, Set.restrict_eq_restrict_iff, SimplicialMap.comp] at gf_id
...
theorem simplicial_iso_vertices (X : SimplicialComplex α) (Y : SimplicialComplex β)
    (f : SimplicialMap X Y) : IsSimplicialIso f → vertices Y = f.map '' vertices X :=
  by
  intro f_iso
  unfold IsSimplicialIso at f_iso
  choose g gf_inv using f_iso
...
noncomputable instance IsSimpliciallyIso.fintype (X : SimplicialComplex α) [Fintype X.simplices]
    (Y : SimplicialComplex β) (X_iso_Y : X ≅ Y) : Fintype Y.simplices :=
  by
...
theorem simplicialCoe_inv_is_simplicial [Nonempty α] {X : SimplicialComplex α}
    (φ : SimplicialCoe X β) : IsSimplicialMap (φ[X]) X (Function.invFunOn φ.coe (vertices X)) :=
  by
  ...
  rw [← coe_s_t]
  have inv_id : Finset.image (Function.invFunOn φ.coe (vertices X)) (Finset.image φ.coe s) = s :=
    by
    simp only [← Finset.coe_inj, Finset.coe_image]
...
noncomputable def simplicialCoeInv [Nonempty α] {X : SimplicialComplex α} (φ : SimplicialCoe X β) :
    SimplicialMap (φ[X]) X :=
  SimplicialMap.mk (Function.invFunOn φ.coe (vertices X)) (simplicialCoe_inv_is_simplicial φ)

notation φ "⁻ᶜ" => simplicialCoeInv φ

theorem coe_inv_isInverseSimplicialIso [Nonempty α] {X : SimplicialComplex α}
    (φ : SimplicialCoe X β) :
...
theorem coe_is_iso [Nonempty α] (X : SimplicialComplex α) (φ : SimplicialCoe X β) :
    IsSimplicialIso
      (@SimplicialMap.mk α β _ X (φ[X]) φ.coe (by apply map_is_simplicial_onto_image)) :=
  by
  unfold IsSimplicialIso
  use@simplicialCoeInv _ _ _ _ _ X φ
  apply coe_inv_isInverseSimplicialIso

theorem simplicialCoeInv_inj [Nonempty α] {X : SimplicialComplex α} (φ : SimplicialCoe X β) :
    Set.InjOn φ⁻ᶜ.map (vertices (φ[X])) :=
  by
...
theorem SimplicialCoe.iso_onto_image [Nonempty α] {X : SimplicialComplex α}
    (φ : SimplicialCoe X β) : X ≅ φ[X] :=
  by
  unfold IsSimpliciallyIso
  use φ.coe
...
theorem coe_preserves_iso [Nonempty α] (X Y : SimplicialComplex α) (φ : SimplicialCoe X β)
    (ψ : SimplicialCoe Y β) : X ≅ Y → φ[X] ≅ ψ[Y] :=
  by
  intro X_iso_Y
...
theorem coe_comp_is_injective {X : SimplicialComplex α} (φ : SimplicialCoe X β)
    (ψ : SimplicialCoe (φ[X]) γ) : Set.InjOn (ψ.coe ∘ φ.coe) (vertices X) :=
  by
  apply Set.InjOn.comp ψ.injective φ.injective
  simp only [Set.MapsTo]
  intro x x_vert
  dsimp only [simplicialImage, vertices, SimplicialComplex.simplices]
...
theorem coe_comp_image (X : SimplicialComplex α) (φ : SimplicialCoe X β)
    (ψ : SimplicialCoe (φ[X]) γ) :
    (simplicialImage X (ψ.coe ∘ φ.coe)).simplices = (simplicialImage (φ[X]) ψ.coe).simplices :=
  by
  dsimp only [simplicialImage, SimplicialComplex.simplices]
  rw [Set.ext_iff]
  intro s
  constructor
  · intro s_in_comp
...
@[simp]
def SimplicialCoe.comp {X : SimplicialComplex α} (φ : SimplicialCoe X β)
    (ψ : SimplicialCoe (φ[X]) γ) : SimplicialCoe X γ :=
  SimplicialCoe.mk (ψ.coe ∘ φ.coe) (coe_comp_is_injective φ ψ)

-- Restriction of coe is coe.
theorem coe_restrict_is_injective (X Y : SimplicialComplex α) (φ : SimplicialCoe X β)
    (Y_subcomp_X : vertices Y ⊆ vertices X) : Set.InjOn φ.coe (vertices Y) :=
  by
  apply Set.InjOn.mono Y_subcomp_X
  apply φ.injective

@[simp]
def SimplicialCoe.restrict {X : SimplicialComplex α} (φ : SimplicialCoe X β)
    (Y : SimplicialComplex α) (Y_subcomp_X : vertices Y ⊆ vertices X) : SimplicialCoe Y β :=
  SimplicialCoe.mk φ.coe (coe_restrict_is_injective X Y φ Y_subcomp_X)

notation φ "[" Y "; " H "]" => simplicialImage Y (SimplicialCoe.restrict φ Y H).coe

def SimplicialCoe.restrictCoe {X Y : SimplicialComplex α} {Y_subcomp_X : vertices Y ⊆ vertices X} :
    Coe (SimplicialCoe X β) (SimplicialCoe Y β) :=
  Coe.mk fun φ : SimplicialCoe X β => φ.restrict Y Y_subcomp_X

-- Convert isomorphisms into coercions.
def SimplicialMap.coe {X : SimplicialComplex α} {Y : SimplicialComplex β} (f : SimplicialMap X Y)
    (f_iso : IsSimplicialIso f) : SimplicialCoe X β :=
...
--  ↑     ↗ φ ∘ g
--  Y
def simplicialCoeOnImage {X : SimplicialComplex α} {Y : SimplicialComplex β} (f : SimplicialMap X Y)
    (f_iso : IsSimplicialIso f) (φ : SimplicialCoe Y γ) : SimplicialCoe X γ :=
  SimplicialCoe.mk (φ.coe ∘ f.map)
    (by
      ...
      unfold Set.MapsTo
      intro x x_in_X
      simp only [vertex_iff_singleton, simplicial_iso_implies_lift_bij X Y f f_iso,
...
theorem simplicialCoe_union_simplices (X Y : SimplicialComplex α) (φ : SimplicialCoe (X ∪ Y) β) :
    φ[X ∪ Y].simplices =
      (φ[X; by apply is_subcomplex_vertices; apply subcomplex_simplicial_union_left] ∪
          φ[Y; by apply is_subcomplex_vertices;
            apply subcomplex_simplicial_union_right]).simplices :=
  by apply simplicialImage_union

theorem simplicialCoe_union (X Y : SimplicialComplex α) (φ : SimplicialCoe (X ∪ Y) β) :
    φ[X ∪ Y] ≅
      φ[X; by apply is_subcomplex_vertices; apply subcomplex_simplicial_union_left] ∪
...
theorem simplicialCoe_injective_vertices {X : SimplicialComplex α} (φ : SimplicialCoe X β) :
    Set.InjOn φ.coe (vertices X) := by apply φ.injective

theorem simplicialCoe_surjective_vertices {X : SimplicialComplex α} (φ : SimplicialCoe X β) :
    Set.SurjOn φ.coe (vertices X) (vertices (φ[X])) := by
  simp only [Set.SurjOn, simplicialImage_vertices]

theorem simplicialCoe_mapsTo_vertices {X : SimplicialComplex α} (φ : SimplicialCoe X β) :
    Set.MapsTo φ.coe (vertices X) (vertices (φ[X])) :=
  by
...
theorem simplicialCoe_bijective_vertices {X : SimplicialComplex α} (φ : SimplicialCoe X β) :
    Set.BijOn φ.coe (vertices X) (vertices (φ[X])) :=
  by
  simp only [Set.BijOn]
  constructor
